...
int main() {
   try {
      ...
      netclasses::socket socket(ios);

      socket.connect(config::ip, config::port);

      for(int idx = 0; idx < config::iterations; ++idx) {
         boost::shared_array<char> arr(new char[config::buffer_size]);
         sprintf(arr.get(), "item: %d", idx);

         socket.async_write(arr, config::buffer_size, &write_handler);
      }

      for(int idx = 0; idx < config::iterations; ++idx) {
         socket.async_read(config::buffer_size, &read_handler);
      }
